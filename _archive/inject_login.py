...
# -*- coding: utf-8 -*-
"""
inject_login.py — Injector da DLL CABAL_Login.dll
Injeta CABAL_Login.dll no CabalMain.exe (aguarda processo aparecer).
Uso (Admin): python inject_login.py [--wait]
O jogo deve estar NA TELA DE LOGIN. Aperte F8 no jogo para iniciar o login.
"""
...
def inject_dll(pid, h_process, dll_path, dll_name):
    ...
    if is_wow64.value:
        raise RuntimeError('Processo 32-bit — DLL x64')
    ...
    try:
        ...
        h_thread = k32.CreateRemoteThread(h_process, None, 0,
                                          ctypes.cast(p_loadlib, ctypes.c_void_p), remote_mem, 0, None)
        if not h_thread:
        ...
        try:
            ...
            if exit_code.value == 0:
                raise RuntimeError('LoadLibraryA retornou 0')
...
DLL_PATH = os.path.join(PROJECT_DIR, 'x64', 'Release', 'CABAL_Login.dll')

if not os.path.exists(DLL_PATH):
    print(f'[ERRO] CABAL_Login.dll nao encontrada: {DLL_PATH}')
    input('Pressione Enter para sair...')
    sys.exit(1)

print('=== CABAL_Login — Injector ===')
...
if pid:
    ...
    print('[+] Abra o jogo normalmente (launcher/atalho)')
    print('[+] Vou injetar CABAL_Login.dll assim que o processo aparecer')
    print('[+] DepoIs: foco no JOGO e aperte F8 para iniciar o login')
    while True:
        pid = find_cabalmain_pid()
        if pid:
            ...
            if h_process:
                ...
                print(f'[-] OpenProcess falhou, tentando...')
        time.sleep(1)

try:
    if not inject_dll(pid, h_process, DLL_PATH, 'CABAL_Login.dll'):
        sys.exit(1)
    print()
    print('[+] CABAL_Login.dll INJETADA com sucesso!')
    print(f'[+] PID: {pid}')
    print('[+] Dica: login.cfg já tem as coordenadas (Venus/Canal 9/Entrar/Começa)')
    print('[+] Na tela de login: FOCO NO JOGO e aperte F8 para iniciar o login')
    print('[+] Resultado: output.txt + login_0.log')
